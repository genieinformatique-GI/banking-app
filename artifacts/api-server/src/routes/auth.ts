...
router.patch("/me", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  try {
    const { firstName, lastName, phone, country, dateOfBirth, hasInvested, previousBroker, previousAmount } = req.body || {};
    ...
    if (hasInvested !== undefined) updates["hasInvested"] = hasInvested;
    if (previousBroker !== undefined) updates["previousBroker"] = previousBroker;
    if (previousAmount !== undefined) updates["previousAmount"] = previousAmount;

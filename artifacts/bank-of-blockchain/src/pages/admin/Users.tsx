...
export default function AdminUsers() {
  ...
  const [newUser, setNewUser] = useState({
    ...
    adminRole: "", adminPermissions: {} as Record<string, boolean>, hasInvested: false, previousBroker: "", previousAmount: "",
  });
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCreateOpen(false);
      setNewUser({ email: "", password: "", firstName: "", lastName: "", phone: "", country: "", dateOfBirth: "", role: "user", status: "active", adminRole: "", adminPermissions: {}, hasInvested: false, previousBroker: "", previousAmount: "" });
    } catch (err: any) {
            ...
            <div className="space-y-3 border border-muted rounded-lg p-4 bg-muted/20">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer"><input type="radio" checked={newUser.hasInvested === true} onChange={() => setNewUser(p => ({ ...p, hasInvested: true }))} /> Oui</label>
                  <label className="flex items-center gap-2 cursor-pointer"><input type="radio" checked={newUser.hasInvested === false} onChange={() => setNewUser(p => ({ ...p, hasInvested: false }))} /> Non</label>
                </div>
              </div>
              {newUser.hasInvested && (
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <Input placeholder="Ex: Binance, eToro..." value={newUser.previousBroker} onChange={e => setNewUser(p => ({ ...p, previousBroker: e.target.value }))} />
                  </div>
                  <div className="space-y-1">
                ...
                <div className="space-y-3 border border-muted rounded-lg p-4 bg-muted/20">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-muted/40 rounded-lg p-3">
                      <p className="text-xs text-muted-foreground">A déjà investi</p>
                      <p className="font-medium mt-0.5">{selectedUserDetail.hasInvested ? "Oui" : "Non"}</p>
                    </div>
                    {selectedUserDetail.previousBroker && (
                      <div className="bg-muted/40 rounded-lg p-3">
                        <p className="text-xs text-muted-foreground">Plateforme / Broker</p>
                        <p className="font-medium mt-0.5">{selectedUserDetail.previousBroker}</p>
                      </div>
                    )}
